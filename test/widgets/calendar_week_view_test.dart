...
void main() {
  testWidgets('shows a 3-day window and a swipe advances by a full 3 days',
      (tester) async {
    // 2026-06-10 is a Wednesday → first window Tue 9 / Wed 10 / Thu 11.
    ...
    // Header reflects the first 3-day window.
    expect(find.textContaining('Tue 9'), findsWidgets);
    expect(find.textContaining('Thu 11 Jun'), findsWidgets);

    // Swipe left → next full window Fri 12 / Sat 13 / Sun 14 (advances by 3).
    await tester.drag(find.byType(PageView), const Offset(-500, 0));
    await tester.pumpAndSettle();

    expect(find.textContaining('Fri 12'), findsWidgets);
    expect(find.textContaining('Sun 14 Jun'), findsWidgets);
